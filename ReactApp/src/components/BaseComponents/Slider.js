...
function SliderComponent(props) {
  const emitChangeDebounced = useRef(debounce((value) => emitChange(value), 10))
    .current;
  ...
  let minVal = props.min !== undefined ? parseFloat(props.min) : 0;
  let maxVal = props.max !== undefined ? parseFloat(props.max) : 100;
  let minPos = minVal;
  let maxPos = maxVal;
  ...
  if (props.logarithmic && minVal > 0) {
    ...
    let maxLog = Math.log(maxVal);
    scale = 100 / (maxLog - minLog);
    valuePos = scale * (Math.log(props.value) - minLog);
  }
  ...
  if (props.marks !== undefined) {
    if (props.logarithmic && minVal > 0) {
      ...
      for (let mark in props.marks) {
        marks = {
          ...marks,
          [scale * (Math.log(mark) - minLog)]: props.marks[mark],
        };
      }
    } else {
      marks = props.marks;
            ...
            control={
              ...
              >
                <RCSlider
                  ...
                  value={props.initialized ? parseFloat(valuePos) : 0}
                  min={props.initialized ? parseFloat(minPos) : undefined}
                  max={props.initialized ? parseFloat(maxPos) : undefined}
